...
θ_old = θ,
θ_new = θ,
θ_v = hcat(θ_old*ones(1,2),θ_new*ones(1,s.N-2)),
r_old = r,
r_new = r,
rv = hcat(r_old*ones(1,2),r_new*ones(1,s.N-2)),
β_old = β,
β_new = β,
β_v = hcat(β_old*ones(1,1),β_new*ones(1,s.N-1)),
δ_old = δ,
δ_new = δ,
δ_v = hcat(δ_old*ones(1,1),δ_new*ones(1,s.N-1)),
Pf_old = Pf,
Pf_new = Pf,
Pfv = hcat(Pf_old*ones(1,1),Pf_new*ones(1,s.N-1)),
Yf_old = Yf,
Yf_new = Yf,
Yfv = hcat(Yf_old*ones(1,1),Yf_new*ones(1,s.N-1)),
τ_old = τ,
τ_new = 1*τ,
τ_v = hcat(τ_old*ones(1,2),τ_new*ones(1,s.N-2)),
τ_m_c_old = τ_m_c,
τ_m_c_new = 1*τ_m_c,
τ_m_c_v = hcat(τ_m_c_old*ones(1,1),τ_m_c_new*ones(1,s.N-1)),
τ_m_k_old = τ_m_k,
τ_m_k_new = 0.12, #0.27
τ_m_k_v = hcat(τ_m_k_old*ones(1,1),τ_m_k_new*ones(1,s.N-1)),
τ_x_old = τ_x,
τ_x_new = 1*τ_x,
τ_x_v = hcat(τ_x_old*ones(1,1),τ_x_new*ones(1,s.N-1)),
